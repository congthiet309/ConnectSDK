...
-(void) showAlert
{
    ...
    _pairingAlertController = [UIAlertController alertControllerWithTitle:title
                                                                  message:message
    ...
    UIAlertAction *okAction = [UIAlertAction actionWithTitle:ok style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ...
        if (self.pairingType == DeviceServicePairingTypePinCode || self.pairingType == DeviceServicePairingTypeMixed) {
            UITextField *textField = _pairingAlertController.textFields.firstObject;
            NSString *enteredPin = textField.text;
            // Handle the pin code
        }
    }];
    ...
    // If the pairing type requires a pin code input, add a text field
    if (self.pairingType == DeviceServicePairingTypePinCode || self.pairingType == DeviceServicePairingTypeMixed) {
        [_pairingAlertController addTextFieldWithConfigurationHandler:^(UITextField *textField) {
            textField.placeholder = [[NSBundle mainBundle] localizedStringForKey:@"Connect_SDK_Pair_Request_Pin" value:@"Please enter the pin code" table:@"ConnectSDK"];
    ...
    dispatch_on_main(^{
        ...
        [rootViewController presentViewController:_pairingAlertController animated:YES completion:nil];

    });
}

- (void)alertController:(UIAlertController *)alertController didDismissWithButtonIndex:(NSInteger)buttonIndex {
    if (alertController == _pairingAlertController) {
        if (buttonIndex == 0) {
            [self disconnect];
        } else if ((self.pairingType == DeviceServicePairingTypePinCode || self.pairingType == DeviceServicePairingTypeMixed) && buttonIndex == 1) {
...
-(void) showAlertWithTitle:(NSString *)title andMessage:(NSString *)message
{
    ...
    if (!_pinAlertController) {
        _pinAlertController = [UIAlertController alertControllerWithTitle:alertTitle
    ...
    dispatch_on_main(^{
        if (rootViewController.presentedViewController) {
            [rootViewController.presentingViewController presentViewController:_pinAlertController animated:YES completion:nil];
        }
    });
}

- (void)dismissPinAlertView {
    if (_pinAlertController && _pinAlertController.presentingViewController) {
        dispatch_on_main(^{
            [_pinAlertController dismissViewControllerAnimated:NO completion:nil];
            _pinAlertController = nil;
        });
    }
}
...
{
    if (_pairingAlertController && _pairingAlertController.presentingViewController)
        dispatch_on_main(^{
            [_pairingAlertController dismissViewControllerAnimated:NO completion:nil];
            _pairingAlertController = nil;
        });
...
- (void) socketDidConnect:(WebOSTVServiceSocketClient *)socket
{
    ...
    if (_pairingAlertController && _pairingAlertController.presentingViewController) {
            dispatch_on_main(^{
                [_pairingAlertController dismissViewControllerAnimated:YES completion:nil];
            });
        }
